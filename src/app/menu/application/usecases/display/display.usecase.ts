...
export class DisplayUseCase {
    ...
    async getDisplaysByParams(page: number, display: Display) {
        const data = await this.displayRepository.getDisplaysByParams(page, display)
        return data
    }
    ...
    async updateDisplay(display: Display) {
        if (!display.getDisplayId) { 
            throw new Error('No ha sido enviado el ID del menu');
        }
